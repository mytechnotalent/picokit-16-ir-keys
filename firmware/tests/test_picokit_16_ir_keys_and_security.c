/**
 * FILE: test_picokit_16_ir_keys_and_security.c
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 Picokit remote key mapping node:
 * provisioning constants, the packet artifact, CRC, the RYLR998 AT
 * interface, the VS1838B NEC decoder, and the key mapping state machine
 * that pairs each named action with an authenticated heartbeat.
 *
 * BRIEF:
 * Native unit test runner for picokit-15-ir-decode.
 *
 * AUTHOR: Kevin Thomas
 * DATE: September 2026
 */
...
static void assert_pin_constants(void) {
    TEST_ASSERT_EQUAL_UINT(25u, PICOKIT_16_IR_KEYS_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(5u, PICOKIT_16_IR_KEYS_IR_PIN);
    TEST_ASSERT_EQUAL_UINT(8u, PICOKIT_16_IR_KEYS_UART_TX);
    TEST_ASSERT_EQUAL_UINT(9u, PICOKIT_16_IR_KEYS_UART_RX);
}
...
static void assert_frame_constants(void) {
    TEST_ASSERT_EQUAL_UINT(115200u, PICOKIT_16_IR_KEYS_UART_BAUD);
    TEST_ASSERT_EQUAL_UINT(48u, PICOKIT_16_IR_KEYS_FRAME_SIZE);
    TEST_ASSERT_EQUAL_UINT(5000u, PICOKIT_16_IR_KEYS_TX_INTERVAL_MS);
...
static void assert_artifact_ids(void) {
    TEST_ASSERT_EQUAL_UINT(16u, PACKET_NODE_ID);
...
static void assert_artifact_example(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(0x7Du, PACKET_EXAMPLE_FRAME[20]);
    TEST_ASSERT_EQUAL_UINT8(0x00u, PACKET_EXAMPLE_FRAME[21]);
}
...
void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[PICOKIT_16_IR_KEYS_IR_PIN]);
}
...
void test_ir_poll_stuck_high(void) {
    ir_command_t cmd;
    reset_all();
    gpio_put(PICOKIT_16_IR_KEYS_IR_PIN, true);
    TEST_ASSERT_FALSE(ir_remote_poll(&cmd));
}

void test_monitor_build_frame_json(void) {
    char frame[PICOKIT_16_IR_KEYS_FRAME_SIZE];
    g_seq = 12u;
    g_key_code = 0x45u;
    monitor_build_frame(frame, sizeof(frame));
    TEST_ASSERT_EQUAL_STRING("{\"n\":16,\"s\":12,\"k\":69}", frame);
}
...
void test_monitor_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_16_IR_KEYS_LED_PIN]);
}

void test_monitor_action_up(void) {
    init_monitor();
    mock_timer_set_us(1000000u);
    nec_arm_at(0x45u, 1000000u);
    step_at(1000000u);
    TEST_ASSERT_EQUAL_UINT8(0x45u, g_key_code);
}

void test_monitor_action_down(void) {
    init_monitor();
    mock_timer_set_us(1000000u);
    nec_arm_at(0x46u, 1000000u);
    step_at(1000000u);
    TEST_ASSERT_EQUAL_UINT8(0x46u, g_key_code);
}

void test_monitor_action_ok(void) {
    init_monitor();
    mock_timer_set_us(1000000u);
    nec_arm_at(0x47u, 1000000u);
    step_at(1000000u);
    TEST_ASSERT_EQUAL_UINT8(0x47u, g_key_code);
}

void test_monitor_action_none(void) {
    init_monitor();
    mock_timer_set_us(1000000u);
    nec_arm_at(0x44u, 1000000u);
    step_at(1000000u);
...
static void run_monitor_tests(void) {
    ...
    RUN_TEST(test_monitor_action_up);
    RUN_TEST(test_monitor_action_down);
    RUN_TEST(test_monitor_action_ok);
    RUN_TEST(test_monitor_action_none);
