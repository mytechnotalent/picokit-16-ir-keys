...
FRAME_CT = "e0f2ae2cf99379b0c87847f9b6d5bb49b67f5ce63c40"
FRAME_TAG = "0140f49073efeeb077178720b97cb6fb"
PLAINTEXT = b'{"n":16,"s":12,"k":69}'
...
def test_envelope_matches_firmware():
    ...
    ciphertext, tag = crypto._xchacha_seal(
        crypto.derive_field_key(), nonce, bytes([16]), PLAINTEXT)
...
def test_public_aead_round_trip():
    ...
    nonce = bytes(range(24))
    ciphertext, tag = crypto.xchacha_seal(key, nonce, b"\x10", PLAINTEXT)
    opened = crypto.xchacha_open(key, nonce, b"\x10", ciphertext, tag)
